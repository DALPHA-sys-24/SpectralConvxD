...
class SpectralPruning(Layer):
    def __init__(self,
        ...
        super(SpectralPruning, self).__init__(
            activity_regularizer=activity_regularizer, **kwargs)

        self.units = int(units) if not isinstance(units, int) else units
        self.activation = activations.get(activation)
        self.Use_pruning = Use_pruning
        self.percent_pruning = percent_pruning
    ...
    def build(self, input_shape):
        ...
        if self.Use_pruning==False:
            ...
            self.active_neurons = int(self.units * (1 - self.percent_pruning))
            indice=np.sort(np.random.choice(np.arange(self.units), size=self.active_neurons, replace=False))
            indices=list(zip([0 for _ in range(self.active_neurons)],indice))
            self.indices= tf.constant(indices, dtype=tf.int64)
            if self.is_diag_end_trainable:
                self.diag_end = self.add_weight(
                    name='diag_end',
                    shape=(self.active_neurons,),
                    initializer=self.diag_end_initializer,
                    regularizer=self.diag_regularizer,
                    constraint=self.diag_constraint,
                    dtype=self.dtype,
                ...
                self.diag_end = tf.constant(np.random.uniform(low=-0.05,
                                        high=0.05,size=(self.active_neurons,)),
                                        dtype=self.dtype)
        # \lambda_j
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.Use_pruning==True:
            sparse_diag_end = tf.sparse.SparseTensor(indices=self.indices, values= self.diag_end, dense_shape=(1,self.units))
            dense_diag_end = tf.sparse.to_dense(sparse_diag_end)
            kernel = mul(self.base, self.diag_start - dense_diag_end) 
