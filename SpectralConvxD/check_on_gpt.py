class __SpecCnn1d__(Layer):
    def __init__(self, filters,
        ...
        super(__SpecCnn1d__,self).__init__()
    ...
    def build(self, input_shape):
        ...
        if self.padding == 1 and self.stride==1:
            ...
            self.pad=tf.linalg.diag(tf.constant(np.ones((input_shape[1],)),dtype=tf.float32))
            self.indices_phi()
        else:
        ...
        if self.trainable_phi:
            self.kernel = self.add_weight(
                name='phi',
                shape=(self.filters * self.output_shape * self.kernel_size,),
                initializer=self.phi_initializer,
                dtype=tf.float32,
                trainable=self.trainable_phi)
        else: 
            self.kernel = tf.constant(np.ones((self.filters,self.kernel_size)),dtype=tf.float32)
      
        # \lambda_in
        if self.use_lambda_in:
            self.l_in = self.add_weight(name='l_in',
                                    shape=(self.filters,self.kernel_size),
                                    initializer=self.lambda_in_initializer,
                                    dtype=tf.float32,
                                    trainable=self.use_lambda_in)
            self.lambda_in=self.tensor_replication(self.l_in,self.input_shape)     
        else:
        ...
        if self.use_lambda_out:
            self.l_out = self.add_weight(name='l_out',
                                            shape=(self.filters,self.kernel_size),
                                            initializer=self.lambda_out_initializer,
                                            dtype=tf.float32,
                                            trainable=self.use_lambda_out)
            
            self.lambda_out=self.tensor_replication(self.l_out,self.output_shape)
        else:
            self.lambda_out = tf.constant(np.zeros((self.filters,self.output_shape)), dtype=tf.float32)
    ...
    def call(self, inputs):
        ...
        indices=self.indices, values=self.kernel,
    ...
    @tf.function
    def tensor_replication(self,tensor,size_control):
        replication=tf.cast(tf.math.ceil(size_control / tf.cast(self.kernel_size, tf.float32)), tf.int32)
        return tf.tile(tensor, [1, replication])[:, :size_control]
