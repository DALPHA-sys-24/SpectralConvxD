    ...
    if gpus:
        for p in drop.get('p'):
            for order in range(replication['rep']):
                ...
                inv=1
                hyperparameters['feature_select']=True
                ...
                pre_taining_weights,_=models.percentile_spectral_filter(trainable_weights=models.model.get_layer(name='convx').variables,
                                                                    name=models_name[inv],
                                                                    p=p,
                                                                    layer_name='convx')
                ...
                for k,var in enumerate(pre_taining_weights):
                    if var.name=='lambda_in' or var.name=='lambda_in:0':
                        inv=1
                        hyperparameters['feature_select']=False
                        diag_end=var.numpy()
                        break
                ...
                old_weights=models.model.get_layer(name='convx').variables
                for k,var in enumerate(old_weights):
                    if var.name=='lambda_in' or var.name=='lambda_in:0':
                        old_weights[k].assign(diag_end)
                        break
                models.model.get_layer(name='convx').set_weights(old_weights)
                models.train(x_train, y_train, x_test, y_test, name=models_name[inv],verbose=0)
                print(f"Post-training weights diag_end for {models_name[inv]} with p={p}")
                models.evaluate(x_test, y_test, trainable_weights=None, path=path, name=models_name[inv], order=order,layer_name='convx',p=p,pre_pruning=True)
